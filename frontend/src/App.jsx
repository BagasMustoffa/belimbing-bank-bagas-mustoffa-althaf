...
const InputField = ({ label, type, val, setVal }) => ( // Template Input
  <input type={type} placeholder={label} value={val} onChange={(e) => setVal(e.target.value)} required style={{ width: '100%', padding: '12px', marginBottom: '15px', borderRadius: '8px', border: '1px solid #ccc', boxSizing: 'border-box', fontSize: '16px' }} />
);

const SelectDeposito = ({ val, setVal }) => ( // Dropdown Pilihan Deposito
  <div style={{ marginBottom: '15px', textAlign: 'left' }}>
    ...
    <select value={val} onChange={(e) => setVal(parseInt(e.target.value))} style={{ width: '100%', padding: '12px', marginTop: '5px', borderRadius: '8px', border: '1px solid #ccc', fontSize: '16px' }}>
      <option value={1}>Deposito Bronze (3%)</option>
      <option value={2}>Deposito Silver (5%)</option>
      <option value={3}>Deposito Gold (7%)</option>
    </select>
  </div>
);

const Btn = ({ label, onClick, outline = false, type = "button", color = bgGreen }) => ( // Template Button
...
export default function App() { // Aplikasi
  const [view, setView] = useState('HOME'); // Default View
  const [user, setUser] = useState(null); // Inisiasi Awal User
  const [selectedAccount, setSelectedAccount] = useState(null); // Inisiasi Awal Akun
  const [errorMsg, setErrorMsg] = useState(''); // Inisiasi Awal Error
  
  const [name, setName] = useState(''); // Inisiasi Awal Nama
  const [pass, setPass] = useState(''); // Inisiasi Awal Password
  const [confPass, setConfPass] = useState(''); // Inisiasi Awal Confirm Password
  const [depositoId, setDepositoId] = useState(1); // Inisiasi Awal Deposit
  const [amount, setAmount] = useState(''); // Inisiasi Awal Balance
  const [accountName, setAccountName] = useState(''); // Inisiasi Awal Akun User

  const clearForms = () => { // Clear Form
    setName(''); setPass(''); setConfPass(''); setAmount(''); setErrorMsg(''); setAccountName(''); setDepositoId(1);
  };

  const switchView = (target) => { // Switch Halaman
    clearForms();
    setView(target);
  };

  const refreshData = async (userId) => { // Refresh
    try {
      const res = await axios.get(`${API_URL}/customers/${userId}`);
      const updatedUser = res.data.data;
      setUser(updatedUser);
      if (selectedAccount) {
        const updatedAcc = updatedUser.accounts.find(a => a.id === selectedAccount.id);
        if (updatedAcc) setSelectedAccount(updatedAcc);
      }
    } catch (err) { console.error(err); }
  };

  const handleLogin = async (e) => { // Login didalam Halaman Login
    e.preventDefault();
    setErrorMsg('');
    try {
  ...
  const handleRegister = async (e) => { // Register didalam Halaman Register
    e.preventDefault();
    setErrorMsg('');
  ...
  const handleCreateAccount = async (e) => { // Tambah Akun didalam Dashboard
    e.preventDefault();
    try {
  ...
  const handleTransaction = async (type) => { // Transaksi didalam Dashboard
  ...
  const handleUpdate = async (e) => { // Update Data didalam Dashboard
    e.preventDefault();
    if (pass !== confPass) return alert("Confirm Password tidak cocok dengan Password");
    try {
      await axios.put(`${API_URL}/customers/${user.id}`, { name, password: pass });
  ...
  if (view === 'REGISTER') return (
    <CenterBox title="Register Nasabah">
      ...
      <form onSubmit={handleRegister}>
        ...
        <SelectDeposito val={depositoId} setVal={setDepositoId} />
      ...
      <div style={{ width: '100%', marginBottom: '20px' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'center' }}>
          ...
          <tbody>
            ...
            )) : <tr><td colSpan="3" style={{ padding: '20px' }}>Tidak ada akun</td></tr>}
          </tbody>
        </table>
      </div>
      <Btn label="Kembali ke Layar Utama" outline onClick={() => { setUser(null); switchView('HOME'); }} />
    </CenterBox>
  );
  ...
  if (view === 'DASHBOARD') return (
    <div style={{ minHeight: '100vh', width: '100vw', backgroundColor: bgLight, padding: '50px', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      {/* Tambah Akun Baru */}
      <div style={{ width: '700px', backgroundColor: 'white', padding: '30px', borderRadius: '15px', border: `1px solid ${borderCol}`, marginBottom: '30px', textAlign: 'center' }}>
        <h3 style={{ color: bgGreen, marginTop: 0 }}>Tambah Akun</h3>
        <form onSubmit={handleCreateAccount} style={{ maxWidth: '400px', margin: '0 auto' }}>
          <InputField label="Nama Akun" type="text" val={accountName} setVal={setAccountName} />
          <SelectDeposito val={depositoId} setVal={setDepositoId} />
      ...
      {/* Update Data and Logout */}
      <div style={{ width: '700px', backgroundColor: 'white', padding: '30px', borderRadius: '15px', border: `1px solid ${borderCol}`, textAlign: 'center' }}>
        <h3 style={{ color: bgGreen, marginTop: 0 }}>Update Data Pribadi</h3>
        <form onSubmit={handleUpdate} style={{ maxWidth: '400px', margin: '0 auto' }}>
          <InputField label="Nama Baru" type="text" val={name} setVal={setName} />
          <InputField label="Password Baru" type="password" val={pass} setVal={setPass} />
          <InputField label="Confirm Password Baru" type="password" val={confPass} setVal={setConfPass} />
          <Btn label="Update Data" type="submit" />
        </form>
        
        <hr style={{ borderColor: borderCol, margin: '30px 0' }} />
        <button onClick={() => { setUser(null); setSelectedAccount(null); switchView('HOME'); }} style={{ padding: '12px 40px', backgroundColor: '#d93025', color: 'white', border: 'none', borderRadius: '25px', cursor: 'pointer', fontWeight: 'bold' }}>Logout</button>
      </div>
